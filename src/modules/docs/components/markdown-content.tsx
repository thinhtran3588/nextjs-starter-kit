...
const proseClasses = {
  h1: "mt-8 text-3xl font-semibold text-white first:mt-0 sm:text-4xl",
  h2: "mt-8 text-2xl font-semibold text-white sm:text-3xl",
  h3: "mt-6 text-xl font-semibold text-white",
  ...
  li: "leading-relaxed",
  code: "rounded bg-white/10 px-1.5 py-0.5 font-mono text-sm text-white/90",
  pre: "mt-3 overflow-x-auto rounded-xl bg-white/5 p-4",
  blockquote: "mt-3 border-l-4 border-white/20 pl-4 italic text-[var(--text-muted)]",
  a: "text-white underline underline-offset-2 hover:text-white/90",
  strong: "font-semibold text-white",
  hr: "my-8 border-white/10",
  table: "mt-3 w-full border-collapse text-[var(--text-muted)]",
  th: "border border-white/20 bg-white/5 px-3 py-2 text-left font-semibold text-white",
        ...
        components={{
          ...
          code: ({ className: codeClassName, children, ...props }) => {
            ...
            if (isBlock) {
              return (
                <code
                  className="block overflow-x-auto font-mono text-sm text-white/90"
                  {...props}
                >
                  {children}
                </code>
